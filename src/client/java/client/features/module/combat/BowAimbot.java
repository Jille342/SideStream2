...
public final class BowAimbot extends Module
{
    ...
    public void onEvent(Event<?> e) {
        if (e instanceof EventUpdate) {
            ...
            if (Float.isNaN(neededPitch))
                ...
                mc.player.prevPitch = neededPitch;
        }
        super.onEvent(e);
    }
    ...
    public float calculateYawChangeToDst(Entity entity) {
        ...
        if (diffZ < 0.0 && diffX < 0.0) {
            return (float) MathHelper.wrapDegrees(-(mc.player.bodyYaw - (90 + deg)));
        } else if (diffZ < 0.0 && diffX > 0.0) {
            return (float) MathHelper.wrapDegrees(-(mc.player.bodyYaw - (-90 + deg)));
        } else {
            return (float) MathHelper.wrapDegrees(-(mc.player.bodyYaw - Math.toDegrees(-Math.atan(diffX / diffZ))));
        }
    }
